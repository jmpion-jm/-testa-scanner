...
실행: 평일 장중 15분마다 자동 (GitHub Actions, 09:00~15:45 KST) 또는 수동 실행.

⚠️ 2026-09-14: 이 스크립트가 H열에 쓰는 정적값이, 같은 셀의 IMPORTXML
라이브 수식(네이버 스크래핑)을 덮어쓰고 있었음 — 원래 하루 한 번만 돌아서
장중엔 가격이 그대로 고정되는 문제로 사용자가 지적함. IMPORTXML 자체도
그 시점에 재확인해보니 다시 깨져있어서(#N/A) 되돌리는 대신, 이 스크립트의
실행 주기를 하루 1회 → 장중 15분마다로 늘려서 해결함(수식 복구 대신
스크립트 갱신 빈도를 높이는 쪽을 선택 — 네이버 페이지 구조가 계속
바뀌어서 IMPORTXML보다 야후파이낸스가 더 안정적인 소스라고 판단).
"""
