...
def get_universe() -> list[tuple[str, str]]:
    """KOSPI 시총 상위 N종목 (ticker, name) 반환"""
    today = datetime.today()
    for i in range(7):
        d = (today - timedelta(days=i)).strftime('%Y%m%d')
        try:
            cap = krx.get_market_cap_by_ticker(d, market='KOSPI')
            if cap.empty:
                continue
            top = cap.nlargest(UNIVERSE_SIZE, '시가총액')
            result = []
            for t in top.index:
                try:
                    ...
                    result.append((t, t))
            print(f'유니버스 기준일: {d} ({len(result)}종목)')
            return result
        except Exception as e:
            print(f'  유니버스 조회 실패 ({d}): {e}')
    return []
